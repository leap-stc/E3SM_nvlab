...
Note: if you use `user_cpp = '-DMMF_ML_TRAINING -DMMF_NN_EMULATOR'` (meaning both the CRM and NN are active during the simulation), you must set `cb_partial_coupling = '.true.'` otherwise the saved `SOLIN/COSZR` will be wrong.
